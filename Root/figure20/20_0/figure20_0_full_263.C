...
void SaveData(
	...
	// open file
	ofstream myfile;
    myfile.open ("figure20_0_full_263_data.txt");
    myfile << "x1\ty1\tey1\nx2\ty2\tey2";
    for (int i = 0; i < k; ++i)
    {
...
void figure20_0_full_263() {
    ...
    for (int j = 0; j < p; ++j)
    {
        ...
        string file_path = "/media/matthieu/ssd1/Geant4/Full-size/Data_figure20-263/data_";
        ...
        const int MAX_CLUSTER_WIDTH = 3;
        const double CLUSTER_WINDOW = 5.0;
    ...
    auto legend = new TLegend(0.5,0.1,0.9,0.4);
    ...
    legend->AddEntry("gr4","Geant4 full-size module - CBC3","ep");
    legend->Draw();

    gPad->Modified();

    c1->SaveAs("figure20_0_full_263.pdf");
