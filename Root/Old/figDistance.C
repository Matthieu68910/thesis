...
void figDistance() {
    ...
    const int MAX_CLUSTER_WIDTH = 2;
    const int CLUSTER_WINDOW =5;
    const double THRESHOLD = 0.017; // MeV -> = 4 * (1000 * 3.6 keV)
    ...
    // loop on all 3 files
    const int NBR_FILES = 3;
    for (int j = 0; j < NBR_FILES; ++j)
    {
        string file_path = "/media/matthieu/ssd1/Geant4/Data/data_";
	    ...
	    for (int k = 0; k < ENTRIES; k++)
	    {
	        ...
	        if(stub && j == 0)
	        {
	        	nbr_stub1.at(h1->FindBin(momentum)) += 1.;
	        }
	        if(j == 0) 
	        {
	        ...
	        if(stub && j == 1)
	        {
	        	nbr_stub2.at(h2->FindBin(momentum)) += 1.;
	        }
	        if(j == 1) 
	        {
	        ...
	        if(stub && j == 2)
	        {
	        	nbr_stub3.at(h3->FindBin(momentum)) += 1.;
	        }
    ...
    legend->AddEntry("h1","Geant4: d = 2.75 mm","l");
    legend->AddEntry("h2","Geant4: d = 2.65 mm","l");
    legend->AddEntry("h3","Geant4: d = 2.55 mm","l");
    legend->Draw();
